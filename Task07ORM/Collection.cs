...
namespace Task07ORM
{
    public class Collection
    {
        ...
        internal Database InDatabase { get { return inDatabase; } }
        ...
        internal Collection(string cname, XElement schema, Database inDatabase)
        {
            ...
            NamedType[] nt_arr = frecord.Elements()
                ...
                .ToArray();
            var eType = new PTypeRecord(nt_arr);
            ...
            this.cell = new PaCell(new PTypeSequence(eeType), path + cname + ".pac", false);
            if (cell.IsEmpty) cell.Fill(new object[0]);
            this.key_index = new FlexIndex<int>(path + cname + "_id_i", cell.Root, en => (int)en.Field(1).Get(), null);
            // Другие индексы
            foreach (var pair in frecord.Elements().Select((el, ind) => new { el = el, ind = ind }))
            {
                ...
                FlexIndex<int> index = new FlexIndex<int>(path + name_combination + ".pac", cell.Root,
        ...
        //    Person p = person_collection.Element<Person>(code);
        //}
